...
function addTypeToIndex(server) {
    server.post('/addType/:indexName', function (req, res, next) 
		{
		 req.assert('indexName', 'indexName is required and must be alphanumeric string').notEmpty().isAlphanumeric();
		 req.assert('typName', 'typeName is required and must be alphanumeric string').notEmpty().isAlphanumeric();
		 req.assert('payload', 'payload is required and must not be empty').notEmpty();
				...
				esClient.indices.getMapping({index: indexName})
					.then(function (response) {
						...
						esClient.indices.putMapping({
									...
									body: payload //payload //below not required if JSON object passed to create mapping 
									/*	{
										properties: {
											title: { type: "string" },
											content: { type: "string" },
				...
				.then(function (response) {
					console.log('Index ['+indexName+'] Created! Now putting mapping -> '+ JSON.stringify(response));
						...
						esClient.indices.putMapping({
									index: indexName,
									type: typeName, //docType,
									body: payload //paylod //below not required if JSON object passed to create mapping 
									/*	{
										properties: {
											title: { type: "string" },
											content: { type: "string" },
